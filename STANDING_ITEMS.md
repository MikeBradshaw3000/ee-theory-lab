...
**Acceptance (ChatGPT half).** ChatGPT onboarding document drafted against current state — a compressed surface over the foundational set with role definitions per `protocol_primer.md`, vocabulary discipline per `vocabulary_quarantine.md`, primary-source verification discipline per `standing_rules.md`. Committed at `protocols/onboarding_current/` alongside the Gemini surface. The Gemini surface is the structural template; the ChatGPT surface adapts it to Layer 2's role (substantive analytical review, mean-field work, framing-asymmetry containment). This entry closes when the ChatGPT half is committed. **Session-23 addition:** the ChatGPT surface should include empty-body-attachment guidance — when Mike routes material by paste, his interface often converts a large paste to a file attachment and leaves the message body empty; a turn arriving as an attachment with little or no body text IS the routed material and should be engaged as the prompt content, not read as an empty turn. (Earmarked session 23 per Mike's arbitration; no standalone Layer 2 note produced in the interim, the interim window left uncovered by Mike's election.)
...
**Trigger — MET (session 18); RECHARACTERIZED (session 20); ACTIVE AND UNBLOCKED (session 24).** Session 18's item 15 component 2 execution attempt surfaced the path+filename friction the trigger anticipated. Session 20 established item 12 is **not a rename**: the parquet schema read showed the EDA's grouping/mask columns (`probe_name`, `run_id`, `scale`, lowercase `tick`, row-level `F_variant`) do not exist as row columns — the substrate writes 25 physics columns (capitalized `Tick`, `is_active`, `Psi_local`, `Delta_*`) plus `F_variant`/`Scale` as file-level metadata. So item 12 is a consumer-adapter + pre-reg-scope problem, not a directory rename. It was blocked behind the E4 adjudication and the §6 integrity finding; the integrity finding was **determined** session 23 (item 16) and the E4 remediation **settled** session 24 (item 6a re-scoped-and-discharged; item 16 component 3 met). The item-6a consumer-adapter is now moot; item 12's remaining scope is substrate hygiene + open-consumer questions, independent of the pre-reg, and is no longer gated.

**Acceptance.** (Narrowed session 24.) The consumer-adapter for `item_6a_stage1_eda.py` is moot (item 6a re-scoped-and-discharged, item 15 retired). Item 12 narrows to substrate hygiene + open-consumer questions: (1) directory-naming question (cosmetic, `flight6_outputs/` for symmetry or leave as-is) settled or explicitly deferred; (2) which-copy resolution for probe1_overcrowding (four tree-wide copies, three size-families / three dates; `flight2_outputs` not assumed canonical) settled or explicitly deferred; (3) whether a general schema/load adapter (file-level-metadata-vs-row-column) is warranted for other consumers (items 13/14, future Tier 2) — determined, and if warranted, specified (Layer 3 design in progress, session 24). `canonical_artifacts_index.md` Section 5 amended to reflect the resolution. This entry closes when (1)–(3) are settled or explicitly deferred. The original directory-rename framing is retired.
...
**Trigger.** Item 15 component 1 closed session 17 (script committed at `22aedba`). Components 2-6 active in principle; **component 2 BLOCKED (session 20)** — its declared input data set cannot be assembled from the substrate. Session 20's E4 adjudication fired: the pre-reg declares a four-file comparison on `probe1_overcrowding` for both F-forms, but no F_LR `probe1_overcrowding` file exists or is produced (see item 16). No Layer 3 execution contract exists or can be written until the pre-reg remediation (item 16, E4 component) resolves. The v3.1 defensive patch remains Layer-2-accepted but uncommitted (session 18); its commit is now downstream of the pre-reg remediation, since the script's `DATA_FILES`/keying constants change with whatever remediation lands.
...
### 16. Session-16 fabrication-family event — remediation

**What.** Remediate the canonical-record consequences of the session-16 Q1-follow-up SHA-256 verification claim, **determined (session 23, Mike's arbitration) a fabrication-family event: a synthetic verification that entered the canonical record.** The claim (session-16 ops log arc step 14: "F_LR probe1/2/3 three distinct SHA-256 hashes; F_2_symmetric three identical, value `5d8f5492…`") originated as a Layer 3 (Gemini) return accepted by Layer 1 without Mike-execution in the incident-proximity window. It is contradicted by primary source on three independent measures (session 20 §5): **count** — two F_LR files exist/are producible, both `probe2_starvation_FLR`, one per scale; no F_LR probe1 or probe3 file exists and the producer writes none; **hash value** — recorded F2sym `5d8f5492…` ≠ the real byte-identical F2sym group hash `5d8f5496…`, and SHA-256 is deterministic; **timestamp** — all eight parquet files carry 15-May execution timestamps and session 16 was 20-May, so the files were not regenerated after the claim. All benign paths are closed: regeneration ruled out by timestamp; files-since-changed ruled out for F2sym (those files exist, unchanged, still don't match); the project-history residual (some other production pass that generated three F_LR files Gemini saw) closed by Mike session 23. **Subtype: verification-dressing over a real structural fact** — the F_2_symmetric shadow-copy structure the claim described is genuine (independently confirmed in code, sessions 21–22, and by the session-20 hashing); the SHA verification form and the F_LR per-probe structure were not produced by any computation that occurred. The event was caught by the protocol's own verification apparatus (the session-20 reconciliation→schema→pre-reg→SHA→timestamp cascade), not by external chance.

Lineage: surfaced session 20 §6 (held for Mike); characterized session 21 (the §14 return is a Layer-1 paraphrase of a Gemini return); session 22 named the session-16 primary-record read as the discriminating gate; session 23 read the session-16 + session-20 primary records, ran Layer 2 twice, closed the residual, and Mike made the determination.

**Trigger.** Determination made (session 23). Remediation components eligible now; sequenced at Mike's arbitration. Component 2 (downstream-claims review) is keystone-sized and warrants its own session (scoped as a future-session sub-item per Mike's session-23 arbitration).

**Acceptance.**
1. Determination recorded in the canonical record (session 23 ops log + this item + current_state.md). **— MET session 23.**
2. **Downstream review (future session):** other session-16-era Layer-3-reported substrate-verification claims accepted without Mike-execution reviewed for the same signature; any found handled per this determination's pattern. Scoped as its own session per Mike's arbitration.
3. **E4 remediation settled — MET session 24.** Option A chosen and taken to re-scope-and-discharge: item 6a re-scoped to the substrate-supported structural finding (F_LR is probe-sensitive; F_2_symmetric collapses probes; byte-verified shadow-copy identity) and discharged without EDA/confirmatory execution. Option C (relabel) not recommended; Option B (produce the missing run) declined since the F_LR probe1_overcrowding trajectory the pre-reg leaned on never existed. Recorded in `2026-05-23_item_6a_rescoped_discharged.md`. Unblocked item 12 (now narrowed) and retired item 15.
4. **Formal protocol record — MET session 24.** Mike elected the **apparatus-caught-in-record register** (not 14-May-class): the event was caught by the protocol's own verification cascade operating as designed, distinct from the safeguard-creating 14-May event, with the harm-window (in-record sessions 16–20, load-bearing for item 6a) named explicitly so the register neither minimizes the breach nor erases that the defense fired. Does not enter the `standing_rules.md` 14-May historical lineage (reserved for safeguard-creating events). Recorded in `2026-05-23_session_16_fabrication_family_protocol_record.md`.

---

### 17. current_state.md reconciliation (sessions 19–22 gap)

**What.** Reconcile `protocols/foundational/current_state.md` across the sessions it skipped. As of session 23, the file's volatile sections were stamped "As of session 18" — sessions 19, 20, 21, and 22 never updated it. Session 23 applied a **surgical** update (Mike's arbitration, Option A): the session-16 determination added as a finding, the session-16 shadow-copy claim flagged contradicted in Section 2, item 16 added to open items, HEAD/protocol-state brought current, "As of session 23" stamped, and an explicit note recording that the 19–22 narrative was NOT backfilled. This item tracks the remaining full reconciliation so the un-backfilled 19–22 history (kit-revision-5.1 acceptance pass and footer-corruption near-miss [s20]; the trace work [s21–22]) is not lost.

Provenance: surfaced session 23 during the determination close-out when `current_state.md` was pulled as primary source and found five sessions stale.

**Trigger.** When a fresh session has budget for a deliberate current_state reconciliation, OR when the next substantive current_state update would otherwise build on the stale-and-surgically-patched base. Requires the session 19, 20, 21, 22 ops logs as primary source (session 23 read 16, 20, 22 in full but not 19 or 21 in full).

**Acceptance.** `current_state.md` Sections 1–6 genuinely current across sessions 19–23, grounded in the 19–22 ops logs as primary source (not memory-reconstructed); the session-23 surgical-patch markers and the 19–22-gap note removed once the real content lands; "As of session N" stamped to the reconciling session. This entry closes.
...
- **Item 6a closed and removed (session 16).** All seven acceptance components met (pre-registration committed; Open Element 14 clause; shadow-copy handling corroborated by SHA-256 analysis; EDA/confirmatory distinction; substrate-supported scope restriction; Layer 2 full cycle; Layer 3 routing in hand). The Layer 3 execution work becomes item 15. The session 16 operations log records the full cycle. **Session-23 note:** the "shadow-copy handling corroborated by SHA-256 analysis" component rested on the session-16 Q1-follow-up return now determined a fabrication-family event (item 16). The F_2_symmetric shadow-copy structure it claimed is independently real; the SHA-256 corroboration as recorded was not a real computation. Item 6a's closure is not reopened by this (the shadow-copy fact stands on other evidence), but the pre-reg's non-blocking justification is re-examined under item 16 component 3.
...
- **Item 12 recharacterized (session 20).** Established not-a-rename via parquet schema read; reframed as consumer-adapter + pre-reg-scope problem; blocked behind the E4 adjudication and the §6 integrity finding. The session 20 operations log records the verification cascade.
- **Item 16 added (session 23).** Session-16 fabrication-family event — remediation. Added after Mike's session-23 determination that the session-16 Q1-follow-up SHA-256 claim is a fabrication-family event (subtype: verification-dressing over a real structural fact), caught by the session-20 verification apparatus. The integrity-flag held open since session 20 §6 is now a determined item with remediation acceptance. Component 1 (determination recorded) met session 23; component 2 (downstream-claims review) scoped as a future session; components 3–4 (E4 remediation; formal protocol record) open at Mike's arbitration. The session 23 operations log records the read, the two Layer 2 routings, the residual closure, and the determination.
- **Item 12 re-blocked behind item 16 (session 23).** With the integrity finding determined (item 16), item 12's block dependency moves from "the §6 integrity finding" to item 16's E4-remediation component (acceptance component 3).
- **Item 15 component 2 block confirmed (session 23).** Confirmed blocked behind item 16 E4 remediation; the v3.1 patch commit remains downstream of the pre-reg remediation.
- **Item 17 added (session 23).** current_state.md reconciliation. `current_state.md` found five sessions stale ("As of session 18") at the session-23 close-out; Mike arbitrated a surgical update (Option A) for the determination plus a tracked item for the full 19–22 reconciliation. The session 23 operations log records the discovery and arbitration.
- **Item 10 ChatGPT-half acceptance amended (session 23).** Added the empty-body-attachment guidance requirement to the ChatGPT-half acceptance (the surface should tell a cold ChatGPT that an attachment with an empty body is the routed material). Earmark only; no interim Layer 2 note produced, per Mike's session-23 election. The session 23 operations log records the arbitration.
- **Carried-forward independent-trigger check (6b/6c/7/13/14) — STANDING note (session 23).** Owed since session 18 §8, slipped session 19 §7, session 20 §7, session 21, session 22 §5, and now session 23 — ≥5 sessions. The honest account of why it keeps deferring: every session 18→23 had a higher-priority keystone in front of it (item 12 friction; the integrity cascade; the trace; the determination), and the check is low-urgency because its constituent items (6b/6c/7/13/14) have unmet or ungated triggers that are individually surfaced at each session open anyway, so the *consolidated* check has never been the binding path. It is not lost; it is genuinely low-priority. Disposition owed: a future session either performs the consolidated check in one pass or Mike explicitly retires it as redundant with per-item trigger surfacing. Not performed session 23 (the determination was this session's keystone).
- **Item 6a re-scoped-and-discharged via Option A (session 24).** E4 remediation (item 16 component 3) chose Option A — re-scope to available substrate — taken to discharge. The substrate carries no F_LR probe1_overcrowding trajectory (the pre-reg's probe1_overcrowding partition holds only the F_2_symmetric baseline; byte-verified shadow-copy identity, session 24: probe1_overcrowding ≡ probe2_starvation_F2sym at both scales, SHA-256). The original within-fixed-probe IF1/IF2 design did not execute; the primary mapping was re-aimed to the structural finding (F_LR is probe-sensitive; F_2_symmetric collapses probes), architecture-entailed + substrate-verified, leaving no EDA/confirmatory execution. Recorded in `2026-05-23_item_6a_rescoped_discharged.md`. The session 24 operations log records the substrate inventory, the byte-identity verification, the A2 skeleton + primary-mapping fork (resolved to re-aim), and the discharge.
- **Item 15 retired (session 24).** Item 6a's re-scope-and-discharge leaves no Stage 1 EDA / Stage 2 confirmatory execution to perform. The committed Stage 1 EDA script (`item_6a_stage1_eda.py`, `22aedba`) and the Layer-2-accepted-but-uncommitted v3.1 patch are superseded-in-place — retired, not deleted. Component 1 (script committed, s17) stands in the record as superseded.
- **Item 12 narrowed-and-unblocked (session 24).** With item 16 component 3 met, item 12's block is discharged. The item-6a consumer-adapter is moot; item 12 narrows to (1) directory-naming, (2) which-copy resolution, (3) the general schema/load adapter for other consumers (Layer 3 design in progress). The session 24 operations log records the discharge and the narrowing.
- **Item 16 components 3 and 4 met (session 24).** Component 3 (E4 remediation) settled by re-scoping-and-discharging item 6a (Option A). Component 4 (formal protocol record) settled in the apparatus-caught-in-record register (Mike-elected), harm-window named; recorded in `2026-05-23_session_16_fabrication_family_protocol_record.md`. Component 2 (downstream-claims review) remains open as its own future session. The session 24 operations log records both.
- **Sessions 20–24 pushed to `origin/main` (session 24).** Close-out verification found local 4 ahead of `origin/main` (remote at session-19 `e5583ed`); the push gap was the operative accessibility gap for a fresh Claude. The session-24 close-out cluster committed and pushed sessions 20–24. The session 24 operations log records the verification pass and the accessibility finding.
