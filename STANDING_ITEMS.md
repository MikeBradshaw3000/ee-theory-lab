...
**Trigger — MET (session 18); RECHARACTERIZED (session 20); BLOCKED behind item 16 (session 20→23).** Session 18's item 15 component 2 execution attempt surfaced the path+filename friction the trigger anticipated. Session 20 established item 12 is **not a rename**: the parquet schema read showed the EDA's grouping/mask columns (`probe_name`, `run_id`, `scale`, lowercase `tick`, row-level `F_variant`) do not exist as row columns — the substrate writes 25 physics columns (capitalized `Tick`, `is_active`, `Psi_local`, `Delta_*`) plus `F_variant`/`Scale` as file-level metadata. So item 12 is a consumer-adapter + pre-reg-scope problem, not a directory rename. It was blocked behind the E4 adjudication and the §6 integrity finding; with the integrity finding **determined** session 23 (item 16), item 12 is now blocked behind item 16's E4-remediation component (acceptance component 3) — the pre-reg scope must settle before the consumer-adapter work has a target.

**Acceptance.** (Recharacterized.) Consumer/pre-reg reconciliation settled per item 16's E4 remediation (Option A re-scope to available substrate vs Option B produce missing F_LR probe1_overcrowding run); load-time-adapter for the file-level-metadata-vs-row-column mismatch specified; canonical scripts updated to reference the actual substrate naming and schema (`item_6a_stage1_eda.py` `DATA_FILES`/`REPO_ROOT`, and any others surfaced); `canonical_artifacts_index.md` Section 5 amended to reflect the resolution; the directory-name question (cosmetic, `flight6_outputs/` for symmetry or leave as-is) settled or explicitly deferred; this entry closes. The original directory-rename framing is retired.
...
3. **E4 remediation settled:** item 6a pre-registration's non-blocking justification (which leaned on the contradicted claim) re-examined; E4 remediation chosen — Option A (re-scope the pre-reg to the actually-available substrate; Layer 2's default preference) vs Option B (produce the missing F_LR probe1_overcrowding run, re-register). Option C (relabel) not recommended. Option A is now cleaner since the F_LR probe1_overcrowding trajectory the pre-reg leaned on never existed. Unblocks item 12 and item 15 component 2.
4. **Formal protocol record** of the fabrication-family event (and its apparatus-caught nature) per Mike's arbitration: whether it rises to the 14-May-class register or is logged as an apparatus-caught in-record event is Mike's call.
