...
| C3-SS-001 | Steady-state eligibility apparatus | SS | steady-state-eligibility precondition; windows earned by explicit diagnostics, never assumed | c3_ss_001_battery.py | valid-L2 | L2 measurement validity | TEST_RECORD_C3-SS-001.md | gate for all INT tests |
| C3-OBS-001 | Wave-one substantive probe (Rules A + B) | OBS | first substantive probe; searches for the lifted non-degenerate low/low candidate signature (PROBE_DESIGN_INPUTS_HELD Sec. 4-5) under controlled structural-conduciveness conditions | c3_obs_001_battery.py | valid-L2 | L2 measurement validity | TEST_RECORD_C3-OBS-001.md | wave two (Rule C and beyond) is a separate decision |

The first three are **precondition tests**: they follow necessarily from the Cycle 2 lessons and are not design choices. Each encodes a Cycle 2 lesson as a standing requirement. None of them, on its own, bears on Regime-II-as-structural; they establish the conditions under which a later test's value becomes interpretable. They must clear before any Cycle 3 substantive probe is interpreted. C3-OBS-001 is the first substantive probe; it ran after all three precondition gates cleared, and is recorded at apparatus level (no L4 interpretation attached).
...
### C3-OBS-001 - Wave-one substantive probe (Rules A + B)
**Purpose:** under controlled structural-conduciveness conditions, does the substrate produce the design-anticipated candidate-signature discrimination - and specifically, does any (rule, band, seed, window) produce a lifted, non-degenerate low/low candidate signature? **Apparatus-level only:** read against grid-local nulls and the SS-001-validated earned-window apparatus; no L4 interpretation, the regime question held for Mike / Layer 1. **What wave one covered:** Rule A (both-agree baseline, tau_A sweep) and Rule B (range-bound rule, four bands), 5 seeds each, 60 runs, at the locked 50x50 Moore-1 toroidal topology. Rule C is deferred to wave two. **Findings (apparatus-level, recorded with Layer 2-required amendments):** Rule A non-discriminating (saturation at tau_A in {1,2,3}, collapse at {4..8}; no locked-cluster regime under wave-one conditions, scope-limited). Rule B separated the co-equal observables - eligible-or-near-eligible bands produced positive meanI / negative persistence (anti-clustered), with [2,3] robust, [2,4] earned-but-heterogeneous, [1,2] boundary / near-steady, [3,4] non-eligible / unstable (a fenced non-eligible contrast: opposite, positive persistence). The LowLow_Nondegenerate_Candidate signature did NOT appear on any of the 300 rows - a negative apparatus-level finding recorded as wave-one data. **Gate role:** none downstream by itself; it is the first substantive probe, and its findings seed (but do not settle) wave-two design.

> **Not yet seeded, by design:** the remaining `C3-OBS-*` observable tests (OBS-002 onward) and the `C3-INT-*` integration runs that bear on Regime-II-as-structural. C3-OBS-001 is now seeded and recorded (above); the guard continues to apply to the still-unseeded substantive tests. Those are the substantive design question, handed to the multi-AI design phase. The Flight 2 experience indicates substrate-led design produced cleaner probe structure than architecture-led. Seeding them here would settle by framing what should be settled by engagement. Add them as the design phase produces them.
