# Layer 3 interpretation request - wave-one (A+B) run outputs

From: Layer 1, 2026-05-28.
To: Layer 3.
Purpose: interpretation of the wave-one (A+B) run outputs, per code-production contract Section 3 step 7 (modified delivery flow). Frame: "the run has produced," not "the run demonstrates." This is the apparatus-level interpretation step; L4 readings are not produced at this step.

---

## Section 1 - Routing history (this session)

Per the code-production contract's Section 3 modified delivery flow:

1. Layer 1 code-production contract -> Layer 3.
2. Layer 3 returned code as content.
3. Layer 1 implementation review of v1 - four items requiring correction.
4. Layer 3 returned v2 code addressing all four items.
5. Layer 1 second implementation review - v2 clean.
6. Layer 1 packaged the script via present_files; Mike placed at `cycle3/c3_obs_001_battery.py` (verified: 15,851 bytes, BOM-less UTF-8, LF line endings).
7. Mike's canonical run completed cleanly in the C3-ENV-001 environment.
8. **This step - Layer 3 interpretation of the pasted outputs.**

## Section 2 - State at run-time

- Three Cycle 3 precondition gates cleared and not reopened (C3-ENV-001 passed-L1, C3-CTL-001 valid-L2, C3-SS-001 valid-L2).
- C3-ENV-001 canonical environment active: top-level venv at `C:\Users\vkz244\EE_Theory_Lab\venv` (parent of the repo), Python 3.14.4, numpy 2.4.4, Mesa 3.5.1.
- Topology locked: 50x50, Moore radius 1 (8-neighbor), toroidal.
- Pre-flight verification passed (venv-active, Python version, numpy pin, mesa import, cycle3 reachability).
- Parity check passed (six assertions: Moran's I single, Moran's I batch, SS evaluate_window keys, persistence null z-score, meanI_state null z-score, plus SS Steady_State_Candidate flag - all byte-identical between wave-one's local reimplementations and the CTL-001 / SS-001 apparatus modules on a deterministically seeded test grid).
- 60 NPZ files produced at `cycle3/data_out/c3_obs_001_states_{rule}_{lambda_id}_{seed}.npz` (one per (rule, lambda_id, seed) triple).
- Aggregate CSV produced at `cycle3/data_out/c3_obs_001_results.csv` - 300 window rows (60 runs x 5 candidate windows each) plus header; 21 columns matching the contract Section 6 schema; row counts match the expected dispatch (200 Sweep A rows, 100 Sweep B rows).
...
## Section 4 - What Layer 3 is asked

Apparatus-level interpretation of the pasted outputs. Specifically:

1. **Per-rule, per-Lambda summary.** For each (rule, lambda_id) value, summarize what the run produced across 5 seeds x 5 candidate windows. Use the flag conjunctions and substrate diagnostics. Frame as "the run produced X at this lambda value," not "Rule Y demonstrates Z."

2. **Comparison against design v2 expectations.** The v2 design's expected outcomes (carried forward in the code-production contract Section 4 Edit 6 for Rule A's substrate-grounded picture, and Edit 5 for Sweep B):

   - **Sweep A** expected to be non-discriminating. Per the expanded Rule A meaningful-failure criterion (Edit 2): Rule A fails at the micro-rule level if any tau_A value produces, on 5/5 seeds in earned steady-state windows, either (a) a divergent joint signature or (b) a `LowLow_Nondegenerate_Candidate` signature. Substrate-grounded picture: low tau_A -> uniform saturation (low/low degenerate); intermediate tau_A -> locked clusters (high/high), possibly intermittent freezing; high tau_A -> extinction (low/low degenerate).
   - **Sweep B [2,4]** expected to be tested for the `LowLow_Nondegenerate_Candidate` signature (lifted rho with low/low non-degenerate). Valid alternative outcomes include high/high, high meanI / low persistence, freezing, extinction, or no earned windows.
   - **Sweep B [2,3]** expected to be tested for the divergent signature (high meanI / low persistence). Similar valid alternatives.
   - **Sweep B [1,2]** and **[3,4]** expected to serve as boundary cases framing the structural behavior of the middle bands.

   For each (rule, lambda_id), identify whether the run produced the expected outcome, an unexpected outcome, or no earned steady-state windows.

3. **3/5 and 4/5 patterns.** Per Edit 4 (anti-binary), 5/5 remains the hard meaningful-failure threshold, but 3/5 or 4/5 off-expectation outcomes are design signals worth surfacing, not noise. Where you see such patterns, name them.

4. **Substrate dynamics observed across Rule A.** The path-iii authorization in the code-production contract allowed the substrate to adjudicate where each Rule A regime (uniform saturation, locked clusters, extinction) actually lives. Identify where the substrate placed each regime.

5. **`LowLow_Nondegenerate_Candidate` across the full sweep.** Report the flag's distribution. Where it fires, where it does not, and the underlying diagnostics that drove its True/False value.

## Section 5 - What Layer 3 is NOT asked

- Not asked to declare L1 or L2 clearance on this interpretation. That is Layer 1's judgment at the implementation-review-of-interpretation step.
- Not asked to rule on L4. What the run means for Regime-II-as-structural is held for Mike / Layer 1 on coherence-as-relational-mutual-reinforcement grounds; the apparatus-level interpretation feeds an L4 judgment but does not produce one.
- Not asked to recommend code changes. The code is clean and produced what the contract specified.
- Not asked to reopen any cleared gate.
- Not asked to combine the two SS-001 flags (`Steady_State_Candidate`, `Lifted_Activation_Candidate`) into a single Regime-II-ready flag.
- Not asked to combine the `LowLow_Nondegenerate_Candidate` flag with anything to interpret it as Regime II. It is an apparatus-level flag; the L3-vs-L4 seam stays intact.
- Not asked to ratify the Rule A endpoint amendment recorded in Edit 6 - that's a close-out-cluster matter once interpretation completes.

## Section 6 - Framing discipline

Layer 3's framing held throughout code production. The same discipline applies to interpretation.

- Frame: "the run produced," "the apparatus recorded," "X seeds produced Y," "Sweep A produced no earned steady-state windows where coherence was non-degenerate low/low."
- NOT: "the run demonstrates," "Rule X confirms," "this validates the design," "L2 sufficiency check passed."

L2 clearance on the interpretation is a Layer 1 judgment at the implementation review of the interpretation. If a statement seems to require a stronger claim than the framing allows, weaken the claim, not the framing.

## Section 7 - Routing after Layer 3 returns interpretation

1. Layer 1 review + sufficiency-test (predicted output vs actual; pessimistic-on-passing on each per-Lambda comparison).
2. Layer 2 substantive review.
3. Layer 1 synthesis to canonical record. Test record `TEST_RECORD_C3-OBS-001.md` post-execution block filled from primary source. Registry `C3-OBS-001` row advanced.

No real-substrate L4 INTERPRETATION (Regime-II-as-structural readings) at this step. Apparatus-level interpretation only. L4 readings open at the L4 node and only with Mike / Layer 1 arbitration on relational-mutual-reinforcement grounds.
