...
# -*- coding: utf-8 -*-
"""
Cycle 3, Wave Two: Rule C (Becoming-Active Coupling), M2 Design - NULL EXTENSION
Topology: 50x50, Toroidal, Moore Radius 1
Observables: Psi_meanI_state, Psi_persistence_I (co-equal pair)

Purpose: Null-extension build to generate the 400-tick floor/null family at 
Lambda=0.40, kappa=0. Does not seed CM-1 or CM-0. Emits trajectories for the pure read.
"""
...
def execute_rule_c_m2():
    ...
    for lambda_anchor, kappa_grid in sweeps:
        for target_c, kappa in kappa_grid:
            ...
            for seed in SEEDS:
                ...
                k_str = f"{kappa:+.4f}".replace("-", "m").replace("+", "p").replace(".", "_")
                npz_path = os.path.join(out_dir, f"c3_w2_null_extension_states_L{lambda_anchor}_k{k_str}_s{seed}.npz")
    ...
    csv_path = os.path.join(out_dir, "c3_w2_null_extension_results.csv")
