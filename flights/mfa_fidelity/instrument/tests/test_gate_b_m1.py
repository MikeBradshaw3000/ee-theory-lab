...
def test_reference_refuses_absent_pinned_file(tmp_path):
    with pytest.raises(R.ReferenceError, match="absent"):
        R.load_pinned_reference(str(tmp_path))

@needs_pin
def test_reference_refuses_digest_mismatch(tmp_path):
    # A tampered copy of the pinned tree: same path, one byte changed -> hard fail before exec.
    import shutil, subprocess
    root = tmp_path / "pin"
    subprocess.run(["git", "clone", "-q", "--shared", PIN, str(root)], check=True)
    subprocess.run(["git", "-C", str(root), "checkout", "-q", "4d9a622"], check=True)
    p = root / R.PINNED_RELPATH
    b = bytearray(p.read_bytes()); b[-2] ^= 0x01; p.write_bytes(bytes(b))
    with pytest.raises(R.ReferenceError, match="digest FAILED"):
        R.load_pinned_reference(str(root))
...
@needs_pin
def test_reference_refuses_dirty_source_is_reported_not_hidden(tmp_path):
    """A dirty pinned worktree is RECORDED (digest still gates the bytes); a modified source
    file changes the digest and is refused."""
    root = tmp_path / "pin"
    subprocess.run(["git", "clone", "-q", "--shared", PIN, str(root)], check=True)
    subprocess.run(["git", "-C", str(root), "checkout", "-q", "4d9a622"], check=True)
    (root / "SCRATCH.txt").write_text("dirty")          # untracked file -> worktree dirty, source intact
    r = R.load_pinned_reference(str(root))
    assert r.provenance.worktree_dirty is True and r.provenance.source_file_dirty is False
...
def test_lock_pins_read_from_git_blob_with_frozen_digest_and_mirror_check():
    ...
    assert digest == EXPECTED_LOCK_SHA256 and len(pins) == 20 and mirror is True
...
@needs_pin
def test_reference_read_failure_is_reference_error(monkeypatch):
    real_open = open
    ...
    with pytest.raises(R.ReferenceError, match="unreadable"):
        R.load_pinned_reference(PIN)
