...
---
---
# SOURCE TEXT FOR VERIFICATION: MERGE_SPECIFICATION_v0_3.md
# (verbatim; SHA-256 of the standalone file: dba76c36fa4efc185fe9304453ee3d89e98f028add026c2473fadea2b6f24e03; repo copy at commit 4afa756 is byte-identical)
---

# Merge Specification — MFA-Fidelity Flight Instrument
## Draft v0.3 (all nine L2 required refinements integrated; for Mike's freeze)

**Status:** DRAFT v0.3. Nothing herein authorizes seeding, implementation, or code change. v0.2's ruled items stand unchanged; the deltas from v0.2 are L2's required refinements (ACCEPTED WITH REQUIRED REFINEMENTS, review of 2026-08-19), each integrated at its section and marked **[v0.3/L2-n]** by refinement number. Freeze is Mike's act.
...
- `fixed_count` is **a new merged-instrument implementation, not preservation of B's behavior** — B's legacy-global permutation is not carried; the rebuilt mode uses `Generator.permutation` with declared draw order. Its certification, where needed, is under Gate B's distributional layer (§8.2 B2), never bit-exactness.
...
6.2 **Zero-amplitude no-draw bypass:** at amplitude 0, no stream construction, no draws, no code path — preserving Gate A exactly.
...
All nine L2 required refinements integrated (§§8.1×2, 4.3–4.4, 1.2, 5.1, 7.3+9.2, 8.2, 8.3, 7.2). The lock-file read is complete (§7.4); no L1 action blocks freeze. Sequence: L2 integration verification, scoped — (1) per refinement 1–9, INTEGRATED AS REQUIRED or INTEGRATION DEFECT with location; (2) any new defect introduced by the integration, NONE FOUND complete; (3) freeze-may-proceed confirmation or named blocker — then freeze at Mike's word.
