...
- **"field" — context-dependent (Mike's session-19 arbitration; not a flat ban).** The term splits by workstream:
  - *Manuscript-facing and architectural prose: prohibited.* In architectural prose and in any manuscript-facing language Phil writes from, "field" is prohibited — most sharply as an *explanatory mechanism*. The theory uses Λ, ρ, Ψ as scalar quantities on a discrete substrate; "field" suggests continuum-physics formalism the theory does not adopt as architectural commitment, and "field" as an explanatory mechanism carries mysticism/pseudoscience associations the theory's prose must never invite (Mike's standing concern: any association of the theory with pseudoscience is to be avoided, and social-science invocations of "fields" as explanatory mechanisms are the specific hazard). Use scalar quantity, substrate variable, lattice quantity, or spatial array instead. Lake Vision history aside, "field" in the architectural document is drift toward physics-borrowed framing.
  - *ABM/MFA technical work: available as a genuine technical term.* In the technical workstream (the ABM substrate, mean-field analysis), "field" may be used when it denotes an actual field-theoretic or continuum-limit construction — used with technical precision and where the work or a contract authorizes it. The technical register has legitimate field-theoretic constructions; the prohibition is on the explanatory-mechanism sense and on migration into reader-facing prose, not on precise technical usage.
  - The boundary in one line: technical latitude in the ABM/MFA work; hard discipline against field-as-explanatory-mechanism and against any migration of "field" into architectural or manuscript-facing prose.
...
This document captures the state as of session 19. Items added or amended:
...
- **"field" entry refined from flat prohibition to workstream split (session 19, Mike's arbitration).** The prior entry stated "field" as an unqualified hard prohibition in the physics-borrowed sense. Mike's session-19 arbitration refined it: hard prohibition stands in manuscript-facing and architectural prose (most sharply against field-as-explanatory-mechanism, keyed to Mike's anti-pseudoscience concern), while the ABM/MFA technical workstream retains latitude to use "field" for genuine field-theoretic or continuum-limit constructions used with technical precision. The refinement arose during the session-19 Gemini onboarding surface work, where Layer 2 recommended softening the flat ban and Mike's arbitration produced the workstream split (which Layer 2 then judged sounder than its own recommendation). The kit-revision-5 Section 4 summary reflects this split; this canonical entry is the authoritative statement. The session 19 operations log records the arbitration.

Future updates will be recorded here when committed.

---

— Drafted by Claude as Layer 1 central node, Stage 1 pair-2 of repository restructure, session 9. v2 incorporates Layer 2 review (Open Element discipline addition, "homeostatic imperative" rationale tightening). Section 1 "eligibility" entry, Section 3 prior-cycle cross-reference, Section 4 source-domain scrubs, and Section 5 maintenance log entries added session 10 as deliverable 3 of item 9 reconciliation, integrating prior-cycle vocabulary content into the canonical record. The "field" entry refined from flat prohibition to workstream split in the session 19 close-out cluster per Mike's arbitration. (Note: the "Pending Layer 2 sanity scan before commit" line from the session-9/10 drafting footer is removed as stale — the document was committed during the session-9/10 cluster; its committed state is primary source, the footer was pre-commit drafting language.)